    ...
    <div className="card">
      <div className="pad">
        ...
        <p style={{ margin: "10px 0 0", color: "var(--muted)", lineHeight: 1.6 }}>
          ...
          <b style={{ color: "var(--text)" }}>Spectrum’s Turbo</b>. All other studs
          referenced on this website are outside studs used for past or planned
          breedings and are{" "}
        ...
        <div className="grid">
          <div style={{ gridColumn: "span 12" }}>
            <DogCard
              dog={{
                ...featuredStud,
                registry: featuredStud.registries?.join(" / "),
              }}
        ...
        <div key={idx} style={{ gridColumn: "span 3" }}>
          <Card>
            <img
              ...
              onClick={() => setLightboxSrc && setLightboxSrc(src)}
            />
          </Card>
        </div>
      ))}
...
/** ✅ FIXED: setLightboxSrc is now passed in (so it won’t crash) */
function BreedingBlock({ title, data, setLightboxSrc }) {
  ...
  return (
    <>
      ...
      {data.studHero && (
        <ImageGrid items={[data.studHero]} setLightboxSrc={setLightboxSrc} />
      )}
    </>
  );
}
...
function Studs({ route, setRoute, setLightboxSrc }) {
  const studs = SITE_DATA.ownedStuds.map((s) => ({
    ...s,
    registry: s.registries?.join(" / "),
  }));
    ...
    return (
      ...
      >
        <p style={{ color: "var(--muted)", lineHeight: 1.6, marginTop: 12 }}>
          Turbo is the only owned and standing stud at Bold Heart Bullies. Stud
          services are offered to approved females only and require a signed
          contract.
        </p>
  ...
  if (route.startsWith("/dams/")) {
    ...
    return (
      <Detail dog={dog} setLightboxSrc={setLightboxSrc}>
        ...
        {dog.pendingBreeding && (
          <BreedingBlock
            title="Pending Breeding"
            data={dog.pendingBreeding}
            setLightboxSrc={setLightboxSrc}
          />
        )}
    ...
    <div className="container">
        ...
        <Card key={d.id}>
          <div className="pad">
            ...
            <p style={{ color: "var(--muted)", lineHeight: 1.6 }}>
              Method: {d.pendingBreeding.method} • Dates:{" "}
              {d.pendingBreeding.dates.map(fmtDate).join(", ")} • Status:{" "}
              {d.pendingBreeding.status}
            </p>
            <div className="row">
              <img
                className="thumb"
                style={{ maxWidth: 220, borderRadius: 14 }}
                src={d.hero}
                alt={d.name}
              />
              {d.pendingBreeding.studHero && (
                <img
                  className="thumb"
                  style={{ maxWidth: 220, borderRadius: 14 }}
                  src={d.pendingBreeding.studHero}
                  alt="Outside stud"
                />
              )}
            </div>
      ...
      <div className="section">
        ...
        <p style={{ color: "var(--muted)" }}>
          Planned pairings for future litters. No outside stud pages will be created.
        </p>
      </div>
            ...
            <div className="row">
              ...
              {d.plannedBreeding.studHero && (
                <img
                  className="thumb"
                  style={{ maxWidth: 220, borderRadius: 14 }}
                  src={d.plannedBreeding.studHero}
                  alt="Outside stud"
                />
              )}
            </div>
  ...
  return (
    <div className="container">
      <div className="section">
        <h2>Past Litters</h2>
        <p style={{ color: "var(--muted)" }}>
          Archive only (proof of production). No pricing or availability listed here.
        </p>
      </div>
        ...
        <Card key={d.id}>
          <div className="pad">
            ...
            <p style={{ color: "var(--muted)", lineHeight: 1.6 }}>
              ...
              Litter size: {d.pastLitter.count} ({d.pastLitter.males} males,{" "}
              {d.pastLitter.females} females)
            </p>
    ...
    <div className="container">
      ...
      <Card>
        <div className="pad">
          ...
          <p style={{ color: "var(--muted)", lineHeight: 1.6 }}>
            For puppy inquiries, planned breedings, or stud services (Turbo only), please
            email us with the dog/breeding you’re referencing.
          </p>
        </div>
      </Card>
    </div>
    ...
    <div className="container">
      ...
      <Card>
        <div className="pad" style={{ color: "var(--muted)", lineHeight: 1.7 }}>
          <p>
            <b style={{ color: "var(--text)" }}>{brand.name}</b> respects your privacy.
            This site may collect information you voluntarily provide (such as contacting us
            via email).
          </p>
          <p>
            <b style={{ color: "var(--text)" }}>Information we collect:</b> name/contact
            details you send us, and basic analytics provided by the hosting platform.
          </p>
          <p>
            <b style={{ color: "var(--text)" }}>How we use it:</b> to respond to inquiries,
            manage waitlists (if used), and improve the website.
          </p>
          <p>
            <b style={{ color: "var(--text)" }}>Sharing:</b> we do not sell your
            information. We may share it only when necessary to provide services you request
    ...
    <div className="container">
      ...
      <Card>
        <div className="pad" style={{ color: "var(--muted)", lineHeight: 1.7 }}>
          ...
          <p>
            All photos, logos, and written content are owned by{" "}
            <b style={{ color: "var(--text)" }}>{brand.name}</b> unless otherwise noted and
            may not be reused without permission.
          </p>
          <p>
            Outside studs referenced for planned or past breedings are not owned by Bold Heart Bullies and do not imply availability for services.
          </p>
